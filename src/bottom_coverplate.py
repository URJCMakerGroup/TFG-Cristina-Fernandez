...
class bottom_coverplate(Obj3D):
    ...
    """
    def __init__(self, xtr_lat = 5., xtr_holes = 8., h_bottom = 1., d_bottom = 2., h_rail = 2., h_base = 2., wall_thick = 4., reinf_thick = 3., arduino_w = 55., arduino_d = 105., arduino_h = 5., sides = 10., bolt = 5., chmf_r = 1., axis_h = VZ, axis_d = VX, axis_w = None, pos_h = 1, pos_d = 3, pos_w = 0, pos = V0, name = ''):
        ...
        self.d_o[4] = self.vec_d(sides + xtr_holes/2.)
        self.d_o[5] = self.vec_d(sides + xtr_holes)
        self.d_o[6] = self.vec_d(sides + arduino_d - 3. - xtr_lat)
        self.d_o[7] = self.vec_d(sides + arduino_d + d_bottom - xtr_holes)
        self.d_o[8] = self.vec_d(sides + arduino_d + d_bottom - xtr_holes/2)
        self.d_o[9] = self.vec_d(sides + arduino_d - 3.)
        self.d_o[10] = self.vec_d(sides + arduino_d)
        self.d_o[11] = self.vec_d(sides + arduino_d + d_bottom)
        self.d_o[12] = self.vec_d(sides + arduino_d + d_bottom + sides/2.)
        ...
        self.w_o[3] = self.vec_w(-(arduino_w + 2 * wall_thick - 2.)/2.)
        self.w_o[4] = self.vec_w(-(arduino_w + 2 * wall_thick - 2. + xtr_holes)/2.)
        ...
        for pt_d in (0, 11):
            shp_lat1 = fcfun.shp_box_dir(box_w = xtr_holes, box_d = sides, box_h = self.tot_h, fc_axis_h = axis_h, fc_axis_d = axis_d, cw = 0, cd = 0, ch = 0, pos = self.get_pos_dwh(pt_d, -3, 0))
            cut.append(shp_lat1)
            shp_lat2 = fcfun.shp_box_dir(box_w = - xtr_holes, box_d = sides, box_h = self.tot_h, fc_axis_h = axis_h, fc_axis_d = axis_d, cw = 0, cd = 0, ch = 0, pos = self.get_pos_dwh(pt_d, 3, 0))
            cut.append(shp_lat2)

        for pt_d in (2, 7):
            shp_lat3 = fcfun.shp_box_dir(box_w = xtr_holes, box_d = xtr_holes, box_h = self.tot_h, fc_axis_h = axis_h, fc_axis_d = axis_d, cw = 0, cd = 0, ch = 0, pos = self.get_pos_dwh(pt_d, -3, 1))
            cut.append(shp_lat3)
            shp_lat4 = fcfun.shp_box_dir(box_w = - xtr_holes, box_d = xtr_holes, box_h = self.tot_h, fc_axis_h = axis_h, fc_axis_d = axis_d, cw = 0, cd = 0, ch = 0, pos = self.get_pos_dwh(pt_d, 3, 1))
            cut.append(shp_lat4)

        shp_lat5 = fcfun.shp_box_dir(box_w = xtr_holes, box_d = arduino_d + d_bottom - 2 * xtr_holes, box_h = self.tot_h, fc_axis_h = axis_h, fc_axis_d = axis_d, cw = 0, cd = 0, ch = 0, pos = self.get_pos_dwh(5, -3, 0))
        cut.append(shp_lat5)
        shp_lat6 = fcfun.shp_box_dir(box_w = - xtr_holes, box_d = arduino_d + d_bottom - 2 * xtr_holes, box_h = self.tot_h, fc_axis_h = axis_h, fc_axis_d = axis_d, cw = 0, cd = 0, ch = 0, pos = self.get_pos_dwh(5, 3, 0))
        cut.append(shp_lat6)
        ...
        for pt_w in (-2, 2):
            shp_final = fcfun.shp_filletchamfer_dirpt(shp_final, self.axis_h, fc_pt = self.get_pos_dwh(10, pt_w, 3), fillet = 1, radius = chmf_r)
            for pt_h in (1, 3):
...
shpob_bottom = bottom_coverplate(xtr_lat = 5., xtr_holes = 8., h_bottom = 1., d_bottom = 2., h_rail = 2., h_base = 2., wall_thick = 4., reinf_thick = 3., arduino_w = 55., arduino_d = 105., arduino_h = 5., sides = 8., bolt = 3., chmf_r = 1., axis_h = VZ, axis_d = VX, axis_w = None, pos_h = 1, pos_d = 3, pos_w = 0, pos = V0)